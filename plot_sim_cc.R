...
# Generate data and save concentration curve coordinates -----------------------
sim_cc = function(sigmaU, n = 1000, beta0, beta1) {
  sim_data(sigmaU = sigmaU,
           n = n,
           beta0 = beta0,
           beta1 = beta1) |>
    make_conc_data(outcome_var = Y,
                   rank_var = Xstar,
                   rank_ascend = TRUE)
}
...
all_res_negC = res_sigmaU_100 |>
  ...
  mutate(sim = rep(x = rep(1:1000, each = 1001),
                   times = 4))

# Plot smoothed concentration curves per setting -------------------------------
all_res_negC |>
  ggplot(aes(x = cumprop_num,
             y = cumprop_health,
             color = factor(var_u))) +
  geom_smooth() +
